...
public class DepotSide extends LinearOpMode {
    ...
    void moveMotorsFast(double leftF, double rightF, double leftR, double rightR, double inches,double speed){
        ...
        //Wait until they are done
        double startTime = runtime.seconds();
        while((robot.leftFrontMotor.isBusy() ||robot.rightFrontMotor.isBusy() || robot.leftRearMotor.isBusy() || robot.rightRearMotor.isBusy())&&(runtime.seconds()-startTime)<2){
            robot.leftFrontMotor.setPower(speed);
            robot.rightFrontMotor.setPower(speed);
            robot.leftRearMotor.setPower(speed);
            robot.rightRearMotor.setPower(speed);
    ...
    void moveFast(double inches){
        moveMotorsFast(1,1,1,1,inches,FORWARD_SPEED);
    }
    ...
    void face0(){
        ...
        runtime.reset();
        if(angle > 0) {
            while (imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES).firstAngle > 7.5&& runtime.seconds() < 2) {
                robot.leftFrontMotor.setPower(TURN_SPEED);
                robot.rightFrontMotor.setPower(-TURN_SPEED);
                robot.leftRearMotor.setPower(TURN_SPEED);
                robot.rightRearMotor.setPower(-TURN_SPEED);
